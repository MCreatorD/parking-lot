...
u8 MCU_MessageDeal(u8 *data,u16 RecLen)
{
	...
	switch(data[0])
	{
      ...
      //led_l;//切换成绿色			
		...
		{
			printf("GetSlaveCarrier ack :%d\n",m_slave.Carrier_flg);
			MCU_To_MCU(m_slave.Slave_Addr,GetSlaveCarrierAck);//查询从节点载波开关状态应答
			break;
		}
		...
		case ConfigLedMode://配置灯光模式 ;  主控MCU -> 射频MCU
		{
			//1.关闭定时器
			TIM_Cmd(TIM2, DISABLE);//使能TIMx 
			SelectStep = 0;//2018/11/21  记录错误之前重新定义了
			hasCar = 0;			
      ...
      led_l;//切换成绿色			
		...
		case ConfigLedThreshold://车辆在位检测门槛数值
		{
			...
			flashthreshold(m_slave.readcarthreshold );

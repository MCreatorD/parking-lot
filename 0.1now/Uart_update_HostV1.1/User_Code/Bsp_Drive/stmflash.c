...
{
	FLASH->KEYR=_FLASH_KEY1;	//写入解锁序列.
	FLASH->KEYR=_FLASH_KEY2; 
}
//flash上锁
...
//FLASH_ErasePage(FlashDestination + (PageSize * EraseCounter)); 
void APP_CodeErase(uint32_t app_addr,uint32_t app_size)
{        //80000
	//要擦除的第一个扇区
	u8  SectorStart_num = 0, SectorLast_num = 0;
	u8 i;
	SectorStart_num = STMFLASH_GetFlashSector(app_addr);// ADDR_FLASH_SECTOR_4   ((u32)0x08010000) 	//扇区4起始地址, 64 Kbytes  
	//要擦除的、最后一个扇区
	SectorLast_num = STMFLASH_GetFlashSector(app_addr+app_size);
	//擦除
	for(i = SectorStart_num;i < SectorLast_num;i++)
	{
				STMFLASH_EraseSector(i);
	}
}
//从指定地址开始写入指定长度的数据

   ...
   host_dev_parm.RF_Power = 3600;	
	...
	if(EN25Q256_ReadID() == EN25Q256_ID)//判断Flash是否正常
	{
		...
		}else
		{
					...
					g_pMainApplication->Devicesn = 0x424c120A1E140200;
...
bool read_manage::GetSlave_Carrier(u8 slave_id,unsigned char *value)//返回数值和是否成功
{
	u8 tmp_ack = 0;	
	//检测传入指针安全性
	...
	if(slave_id == 0)//主机ID为0
	{
		host_dev_parm.Carrier_flg = RFPowerRead();
		*value = host_dev_parm.Carrier_flg;//host_dev_parm.;//获取主机载波
		tmp_ack = 1;//查询主机载波
	}else{//从节点
		*value = 0;
		MCU_To_MCU(slave_id,GetSlaveCarrier,0);//查询从节点射频载波状态
		Device_cpu = slave_id;	
		//等待返回
		tmp_ack = waitSlaveAckData(GetSlaveCarrierAck,value,1,500);
	}
	if(1==tmp_ack)
	{
		printf("GetSlave_Carrier %d:%d ok\n",slave_id,*value);	
		return true;
	}	
	printf("GetSlave_Carrier %d:%d false\n",slave_id,*value);	
	return false;	
}
...
bool read_manage::Config_LedThreshold(u8 slave_id,u8 value)
{
	...
	if(slave_id == 0)//主机ID为0
	{
		 ...
		 flashthreshold(readcarthreshold);
		 //if(this->slave_num == 0)//只有主机
	   //{

...
CFrameDecoderStream::getMessage (void)
{
	...
	DeviceSN = get_u64(&g_fdMessageHeader_SN);
	Ver = get_u8(&g_fdMessageHeader_Ver);
	if(1u != Ver)
	{
...
CFrameDecoderStream::getParameter (void)
{
	...
	//m_pDecoder->m_iNext--;
	Type = get_u16(&g_fdParameterHeader_Type);
	...
	if(NULL == pTypeDescriptor)
	{
		...
		pError->m_pRefField = &g_fdParameterHeader_Type;
	...
	*/
	...
	//std::string name = pParameter->m_pType->m_pName;

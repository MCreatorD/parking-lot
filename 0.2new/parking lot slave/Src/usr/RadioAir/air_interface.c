...
uint8_t _6D_T1(void)
{
	...
	if(RF_To_SM(0x01,EPC_Response_Recv_Buf,2))//将标签数据发往安全模块
	{
	   	  temp=(Sm_MessageDeal(19,2));//对串口收取值判断类型，赋值给RxBuffer
        if(0x02==temp)
        {
            _6D_TOKEN1(RxBuffer);
            switch(RxBuffer[7])
            ...
            if(status!=GL_OK)
            {
								RF_To_SM(0x03,EPC_Response_Recv_Buf,2);//通知安全模块，数据传输错误，
							  printf("d");
//							{
...
uint8_t _6D_T2(void)
{
	...
	if(RF_To_SM(0x02,EPC_Response_Recv_Buf,2))
	{
		    temp=(Sm_MessageDeal(19,2));	
        if(temp==0x02)
        {
            _6D_TOKEN2(RxBuffer);
            switch(RxBuffer[7])
            ...
            {
				RF_To_SM(0x03,EPC_Response_Recv_Buf,2);
				printf("a");		
            }
            else
            {
...
#define     CIN     4
#define     CCN     3
uint16_t count1,count2,count3,delaycount;
...
unsigned char _6D_TOKEN1Decode(void)
{
...
unsigned char _6D_EPC_ReadMulti_Protcol(unsigned char RFSwitchoffFlag)
{
        ...
        if (status == GL_OK)
        {
            status_r = _6D_T1();   //读标签EPC 转发到安全模块 ---》 射频
							...
							for(;;)
              {
								...
								{
									status = GL_OK;
//									MCU_To_MCU(m_slave.Slave_Addr,ReadCardEnd);
//									READ_LED_H;
									printf("5");						
									break;
								}
...
uint8_t ReadTag(void)
{
    uint8_t bState = 0;
		...
		//channelSwitchCount++;
		//IQ双路切换 修改切换的规则需求
    bState = _6D_EPC_ReadMulti_Protcol(1);//
		...
		//读卡结束后
	  MCU_To_MCU(m_slave.Slave_Addr,ReadCardEnd);//发送读卡结束 
	  Delay_ms(3);
    return bState;
}

...
#include "soft_spi.h"
#include "common.h"
u16 EN25Q256_TYPE=EN25Q256;	//默认是EN25Q256
u8 flash_Dev_ok = 0; //芯片正常


////要写入到GD25Q127的字符串数组
//const u8 TEXT_Buffer[]={"BL-SR-200"};
...
{	
	u8 temp = 0;
	SerialPutString("init flash\n");
	Soft_SPI_Init();
	//上电读取FLashID 校验芯片在位
	if(EN25Q256_ReadID() == EN25Q256)//判断Flash是否正常
	{
		//Flash 设备正常
		flash_Dev_ok =  flash_Isok;	
    SerialPutString("Flash Dev is ok\n");		
	}else{
		//Flash 设备不正常
		flash_Dev_ok = flash_Notok;
		SerialPutString("Flash Dev is not ok\n");
	}
} 
...
u32 EN25Q256_ReadID(void)
{
	u32 Temp = 0;	  
	EN25Q256_Enable;		
	Soft_SPI_ReadWrite_Byte(EN25Q256_ManufactDeviceID);//发送读取ID命令
	Soft_SPI_ReadWrite_Byte(0x00);//返回第一组8位不要
	Soft_SPI_ReadWrite_Byte(0x00);//返回第二组8位不要
	Soft_SPI_ReadWrite_Byte(0x00);//返回第三组8位不要   Address (000000h) 
	Temp|=Soft_SPI_ReadWrite_Byte(0xFF)<<8;//Manufacturer ID  返回1CH
	Temp|=Soft_SPI_ReadWrite_Byte(0xFF);//Device ID  
	EN25Q256_Disable;			    
	return Temp;
} 
...
{  
	...
	Soft_SPI_ReadWrite_Byte(EN25Q256_ReadStatusReg); //发送读取状态寄存器命令    
	byte=Soft_SPI_ReadWrite_Byte(0Xff);          //读取一个字节  
...
{   
	EN25Q256_Enable;                            //使能器件   
	Soft_SPI_ReadWrite_Byte(EN25Q256_WriteStatusReg);//发送写取状态寄存器命令    
	Soft_SPI_ReadWrite_Byte(sr);               	//写入一个字节  
	EN25Q256_Disable;                           //取消片选     	      
}   
//EN25Q256使能	 (06H)
//将WEL置位   
...
{
	EN25Q256_Enable;                          	//使能器件   
  Soft_SPI_ReadWrite_Byte(EN25Q256_WriteEnable); 	//发送写使能  
	EN25Q256_Disable;                          	//取消片选     	      
} 
//EN25Q256写禁止	 (04h)
//将WEL清零  
...
{  
	EN25Q256_Enable;                            //使能器件   
    Soft_SPI_ReadWrite_Byte(EN25Q256_WriteDisable);  //发送写禁止指令    
	EN25Q256_Disable;                             //取消片选     	      
}
...
{ 
 	u16 i;   										    
	EN25Q256_Enable;                            	//使能器件   
    Soft_SPI_ReadWrite_Byte(EN25Q256_ReadData);         	//发送读取命令   
    Soft_SPI_ReadWrite_Byte((u8)((ReadAddr)>>16));  	//发送24bit地址    
    Soft_SPI_ReadWrite_Byte((u8)((ReadAddr)>>8));   
    Soft_SPI_ReadWrite_Byte((u8)ReadAddr);   
    for(i=0;i<NumByteToRead;i++)
	{ 
        pBuffer[i]=Soft_SPI_ReadWrite_Byte(0XFF);   	//循环读数  
  }
	EN25Q256_Disable;  				    	      
}  
...
void EN25Q256_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
    ...
    EN25Q256_Write_Enable();                  	//SET WEL 
		EN25Q256_Enable;                           	//使能器件   
    Soft_SPI_ReadWrite_Byte(EN25Q256_PageProgram);      	//发送写页命令   
    Soft_SPI_ReadWrite_Byte((u8)((WriteAddr)>>16)); 	//发送24bit地址    
    Soft_SPI_ReadWrite_Byte((u8)((WriteAddr)>>8));   
    Soft_SPI_ReadWrite_Byte((u8)WriteAddr);   
    for(i=0;i<NumByteToWrite;i++)Soft_SPI_ReadWrite_Byte(pBuffer[i]);//循环写数  
	EN25Q256_Disable;                            	//取消片选 
...
{                                   
    ...
    EN25Q256_Wait_Busy();   
  	EN25Q256_Enable;                            	//使能器件   
    Soft_SPI_ReadWrite_Byte(EN25Q256_ChipErase);        	//发送片擦除命令  
	EN25Q256_Disable;                            	//取消片选     	      
...
{  
	//监视falsh擦除情况,测试用   
 	//SerialPutString("fe:%X\r\n",Dst_Addr);	  
 	Dst_Addr*=4096;
    EN25Q256_Write_Enable();                  	    //SET WEL 	 
    EN25Q256_Wait_Busy();   
  	EN25Q256_Enable;                            	//使能器件   
    Soft_SPI_ReadWrite_Byte(EN25Q256_SectorErase);      	//发送扇区擦除指令 
    Soft_SPI_ReadWrite_Byte((u8)((Dst_Addr)>>16));  	    //发送24bit地址    
    Soft_SPI_ReadWrite_Byte((u8)((Dst_Addr)>>8));   
    Soft_SPI_ReadWrite_Byte((u8)Dst_Addr);  
		EN25Q256_Disable;                           //取消片选     	      
...
{ 
  	EN25Q256_Enable;                           	 	//使能器件   
    Soft_SPI_ReadWrite_Byte(EN25Q256_PowerDn);         //发送掉电命令  
	EN25Q256_Disable;                            	//取消片选     	      
   //delay_us(3);                                    //等待TPD  
}   
//唤醒
...
{  
  	EN25Q256_Enable;                            	//使能器件   
    Soft_SPI_ReadWrite_Byte(EN25Q256_ReleasePowerDown);	//  send W25X_PowerDown command 0xAB    
	  EN25Q256_Disable;                            	//取消片选     	      
    // delay_us(3);                            	//等待TRES1
}   

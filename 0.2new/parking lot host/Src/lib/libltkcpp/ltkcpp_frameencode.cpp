  ...
  const CElement *              pElement)
{
	...
	if(m_pRefType->m_bIsMessage){
		...
		devicesn = ((CMessage *)pElement)->getDeviceSN();
		put_u64(devicesn,&g_fdMessageHeader_SN);

		llrp_u8_t ver;
		ver = 1;//�б��У�ȡֵ�̶�Ϊ1
		put_u8(ver,&g_fdMessageHeader_Ver);

		llrp_u16_t type;
		type = m_pRefType->m_TypeNum;
		put_u16(type, &g_fdMessageHeader_Type);
		put_u32(0, &g_fdMessageHeader_Length);//��Ϣ�ܳ��ݶ�Ϊ0����������ʵ�ʵ��ܳ�
		put_u32(((const CMessage *)pElement)->getMessageID(),
			&g_fdMessageHeader_MessageID);
	}else{	
		put_u16(m_pRefType->m_TypeNum, &g_fdParameterHeader_Type);
		put_u16(0, &g_fdParameterHeader_Length);
	}
	...
	if(m_pRefType->m_bIsMessage){
		assert(nLength >= 19); //要求长度至少为19，否则程序结束�
		pLen += 11;
		//根据规范，实际放入的长度不包括消息头长度，故应减去消息头长度
		nLength -= 19;
		pLen[0] = nLength >> 24u;
		pLen[1] = nLength >> 16u;
		pLen[2] = nLength >> 8u;
		...
		assert(nLength >= 4); //要求长度至少为4,否则程序结束
		pLen += 2;
		//根据规范，实际放入的长度不包括消息头长度，故应减去消息头长度
		nLength -= 4;
		pLen[0] = nLength >> 8u;

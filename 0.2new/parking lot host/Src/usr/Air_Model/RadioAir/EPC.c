...
uint8_t TRcal_timer;
uint8_t EPC_CRC[2];
uint8_t EPC_Response_Recv_Buf[200];
...
uint8_t EPC_Response_Recv_Bufx[200];
...
//uint8_t EPC_Response_Recv_Bufx_Q[200];
//uint8_t EPC_Response_Recv_Buf_Q[200];
//uint8_t EPC_Recv_Bit_Indexx_Q;
//uint16_t EPC_Premablex_Q;
//uint8_t EPC_Recv_Stagex_Q;
//u32 EPC_periodx_Q;
//uint8_t EPC_Temp_Recv_Bytex_Q;
//uint16_t EPC_ResponseRecvBitsx_Q;
//uint16_t EPC_Recv_Byte_Indexx_Q;
//uint16_t EPC_ResponseRecvBits_Q ;
void EPC_IO_Configure(void)
{
	GPIO_InitTypeDef  GPIO_InitStructure;
	
	RCC_AHB1PeriphClockCmd(ASK_DATA_CLK, ENABLE);		 //前向命令
	GPIO_InitStructure.GPIO_Pin = ASK_DATA_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
	GPIO_Init(ASK_DATA_PORT, &GPIO_InitStructure);
	//Set_ASK_DATA();
	

	
	RCC_AHB1PeriphClockCmd(IFSW_CTR_CLK, ENABLE);   //前后相隔离

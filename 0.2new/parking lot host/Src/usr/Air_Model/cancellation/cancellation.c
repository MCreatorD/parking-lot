...
//	}
	...
	for(int angle=0;angle<360;angle+=angle_step){
		...
		Ipini = DA_2V5;
		Inini = DA_2V5;
		setDACValue(Qpini,QP_CHANNEL);
		setDACValue(Qnini,QN_CHANNEL);
		setDACValue(Ipini,IP_CHANNEL);
		setDACValue(Inini,IN_CHANNEL);
		Delay_ms(AD_TIME);
		...
		for(int i=0;i<50;i++){
			...
			//squ = squ>>16;
			int32_t delta_I = (I_value*cos(x)+Q_value*sin(x))/50.0f;
			int32_t delta_Q = (Q_value*cos(x)-I_value*sin(x))/50.0f;
			...
			if(Qp_DAValue>900){
				Qp_DAValue = 900;
			}else if(Qp_DAValue<100){
				Qp_DAValue = 100;
			}
			if(Qn_DAValue>900){
				Qn_DAValue = 900;
			}else if(Qn_DAValue<100){
				Qn_DAValue = 100;
			}
			if(Ip_DAValue>900){
				Ip_DAValue = 900;
			}else if(Ip_DAValue<100){
				Ip_DAValue = 100;
			}
			if(In_DAValue>900){
				In_DAValue = 900;
			}else if(In_DAValue<100){
				In_DAValue = 100;
			}
			Qpini = Qp_DAValue;
			Qnini = Qn_DAValue;

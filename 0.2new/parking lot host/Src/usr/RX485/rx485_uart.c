 ...
 */
  ...
  while(USART_GetFlagStatus(RX485_USARTx, USART_FLAG_TC)==RESET);//解决第一个字节丢失问题
...
void RS485_IO_Init(void)//485 收发控制脚 上电默认为接收
{
 GPIO_InitTypeDef  GPIO_InitStructure;
...
static void __485_delay(unsigned int rx485_count)
{
	for(;rx485_count>0;rx485_count--);
}
...
int RX485USARTx_SendData(unsigned char *buf,int buflen)
{
		...
		//切换收发状态的时候前后微弱延时减小总线不稳定时数据发送的数据丢失
		__485_delay(100);
		RS485_TX_H;
	  RS485_RX_H;
		//USART_ClearFlag(RX485_USARTx,USART_IT_RXNE);
		//USART_ITConfig(RX485_USARTx,USART_IT_RXNE,DISABLE);
		__485_delay(100);
